...
| --- | --- | --- | --- | --- | --- | --- | --- |
| visual only (keyframes) | 0.53 | 0.90 | **0.72** | 0.603 | 0.925 | — | 287 ms |
| joint vector only (image+speech) | 0.73 | 0.93 | **0.83** | 0.801 | 0.956 | — | 67 ms |
| full-text only (Atlas Search BM25) | 0.60 | 0.30 | **0.45** | 0.652 | 0.360 | — | 59 ms |
| transcript only · autoEmbed voyage-4 | 0.83 | 0.23 | **0.53** | 0.886 | 0.365 | — | 132 ms |
| transcript only · client voyage-4 | 0.83 | 0.20 | **0.52** | 0.886 | 0.347 | — | 260 ms |
| transcript only · client, voyage-4-lite queries | 0.83 | 0.23 | **0.53** | 0.889 | 0.366 | — | 265 ms |
| transcript + rerank | 0.90 | 0.43 | **0.67** | 0.937 | 0.522 | 0.78 | 439 ms |
| fixed fusion, equal weights, no rerank | 0.57 | 0.57 | **0.57** | 0.721 | 0.672 | 0.57 | 183 ms |
| fixed fusion, equal weights + rerank | 0.70 | 0.57 | **0.63** | 0.799 | 0.661 | 0.71 | 471 ms |
| fixed fusion, tuned weights + rerank | 0.80 | 0.50 | **0.65** | 0.853 | 0.600 | 0.75 | 476 ms |
| adaptive routing · autoEmbed | 0.83 | 0.80 | **0.82** | 0.888 | 0.850 | 0.76 | 475 ms |
| **scene-first (default)**: joint vector ranks, reranker picks the second | 0.73 | 0.93 | **0.83** | 0.801 | 0.956 | 0.73 | 255 ms |
| adaptive · client-side fusion | 0.83 | 0.80 | **0.82** | 0.888 | 0.850 | 0.76 | 905 ms |
| adaptive · client transcript mode | 0.83 | 0.80 | **0.82** | 0.888 | 0.850 | 0.76 | 628 ms |
...
**Reading this table.** One joint image+speech vector per scene (mean 0.83) beats fusing the same signals after retrieval (best tuned fusion 0.65): it wins 14 questions the fusion misses and loses 3 (exact McNemar p = 0.013). Separate lists disagree on every question that is about only one of the two, and rank fusion averages the disagreement away; a joint vector never produces it. The default, scene-first, ranks with that vector and uses the reranker only to pick the second (Moment@1 0.73), in 255 ms. Adaptive routing, which repairs late fusion by choosing a specialist per question, ties it (4 vs 3, p = 1.000) at 475 ms: it leans ahead on questions about what was said, behind on what was shown.
...
| --- | --- | --- | --- | --- | --- |
| keyframes only | 0.12 | 0.88 | **0.50** | — | 79 ms |
| transcript + rerank | 0.55 | 0.07 | **0.31** | 0.86 | 359 ms |
| rank fusion, tuned weights + rerank | 0.33 | 0.10 | **0.21** | 0.85 | 480 ms |
| joint vector only | 0.35 | 0.90 | **0.62** | — | 65 ms |
| adaptive routing | 0.50 | 0.68 | **0.59** | 0.85 | 455 ms |
| scene-first | 0.35 | 0.90 | **0.62** | 0.71 | 223 ms |
...
**Decision rule, fixed before this corpus was run:** scene-first becomes the default if it is not significantly worse than adaptive routing on either corpus and it is faster. On this corpus it is not significantly worse (14 vs 11, p = 0.690) and faster (223 vs 455 ms).
