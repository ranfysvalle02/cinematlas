...
def test_fixed_routing_keeps_classic_fusion(indexed):
    results = indexed.search("sonic boom", top_k=3, routing="fixed")
...
def test_explicit_weights_disable_routing(indexed):
    results = indexed.search("sonic boom", top_k=3, weights={"visual": 1})
...
def test_sources_can_be_restricted(indexed, fake_mongo):
    indexed.search("sonic boom", sources=("transcript",), rerank=False)
...
def test_missing_scene_index_is_skipped(indexed, fake_mongo):
    ...
    results = indexed.search("sonic boom")
...
@pytest.mark.parametrize("kwargs", [{"top_k": 0}, {"sources": ("audio",)}])
def test_invalid_arguments(indexed, kwargs):
    with pytest.raises(ValueError):
        indexed.search("q", **kwargs)
...
def test_native_rerank_scores_sentences_server_side(indexed, fake_mongo, fake_voyage):
    ...
    (top, *_r) = indexed.search("how loud was it")
...
def test_text_source_uses_atlas_search_with_video_filter(indexed, fake_mongo):
    indexed.search("X-59 building 4826", sources=("text",), video_id="v", rerank=False)
